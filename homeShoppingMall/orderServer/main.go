...
import (
	...
	"homeShoppingMall/orderServer/dao/redis"
	"homeShoppingMall/orderServer/handler"
	"homeShoppingMall/orderServer/logger"
	"homeShoppingMall/orderServer/pkg/snowflake"
	"homeShoppingMall/orderServer/proto"
	"homeShoppingMall/orderServer/settings"
	"homeShoppingMall/orderServer/utils"
	"net"
	"os"
	"os/signal"
	"syscall"
...
func main() {
	...
	if err := settings.Init(); err != nil {
		zap.L().Error("Main settings.Init failed", zap.Error(err))
		return
	}
	// 2. 初始化日志
	...
	// 初始化商品库存微服务client连接
	if err := consulRegister.GrpcInit(); err != nil {
		zap.L().Error("Main  consulRegister.GrpcInit failed")
	}
	...
	c, _ := rocketmq.NewPushConsumer(
		...
		consumer.WithGroupName("Ame-order"),
	)
